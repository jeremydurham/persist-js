...
var major=swfobject.getFlashPlayerVersion().major;return(major>=8)?true:false;},methods:{init:function(){if(!B.flash.el){var key,el,fel,cfg=C.flash;el=document.createElement('div');el.id=cfg.div_id;fel=document.createElement('div')
fel.id=cfg.id;el.appendChild(fel);document.body.appendChild(el);swfobject.embedSWF(this.o.swf_path||cfg.path,cfg.id,cfg.size.w,cfg.size.h,'8',null,{},cfg.params,{},function(o){B.flash.el=o.ref;});}
...
P._init=true;};P={VERSION:VERSION,type:null,size:0,add:function(o){B[o.id]=o;C.search_order=[o.id].concat(C.search_order);init();},remove:function(id){console.log(id);console.log(C.search_order);var ofs=index_of(C.search_order,id);if(ofs<0){return;}
